...
public class ResponseEntityTestController {
	...
	public ResponseEntity<String> test4(@RequestBody FruitDTO fruitDTO) {
		...
		if (updateResult) {
			return ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body("test4 통과");
		} else {
			return ResponseEntity.status(502).contentType(MediaType.TEXT_PLAIN).body("test4 실패");
		}
